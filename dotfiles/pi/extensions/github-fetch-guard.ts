 ...
 * for private/authenticated GitHub resources. This ensures authenticated
 * access to private repos, pull requests, and GitHub API endpoints.
 */
...
export default function githubFetchGuard(pi: ExtensionAPI) {
  pi.on("before_agent_start", async (event) => {
    ...
    return {
      systemPrompt:
        event.systemPrompt +
        `\n\nIMPORTANT: When working with GitHub URLs, do not use fetch_content for github.com or api.github.com links. Use local shell commands through bash with the authenticated GitHub CLI (gh) instead. Prefer gh for private repos, pull requests, files, reviews, and metadata. If you need PR details, prefer commands such as gh pr view, gh api, git fetch, and git diff.`,
    };
  });
